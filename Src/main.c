...
#define gearRatio 1
#endif
#define S16A_TO_FLOAT 0.0018363952636719
...
/* USER CODE BEGIN PV */
#if MOTOR_NUMBER == 1
	const int INIT_ID 			= 0x001;
	const int DRIVE_ID 			= 0x002;
	const int REF_TENSION_ID 	= 0x003;
	const int TENSION_SENS_ID 	= 0x004;
	const int REQ_ID 			= 0x005;
#elif MOTOR_NUMBER == 2
	const int INIT_ID 			= 0x011;
	const int DRIVE_ID 			= 0x012;
	const int REF_TENSION_ID 	= 0x013;
	const int TENSION_SENS_ID 	= 0x014;
	const int REQ_ID 			= 0x015;
#elif MOTOR_NUMBER == 3
	const int INIT_ID 			= 0x021;
	const int DRIVE_ID 			= 0x022;
	const int REF_TENSION_ID 	= 0x023;
	const int TENSION_SENS_ID 	= 0x024;
	const int REQ_ID 			= 0x025;
#endif


const int ESOTP_ID 			= 0x000;

float Tension_P_gain = 1.5;
float Tension_I_gain = 1.5;
float Tension_D_gain = 0.5;
...
const float Positive_Tension_Limit = 20;
const float Negative_Tension_Limit = 0;
const float Positive_Current_Limit = 4;
const float Negative_Current_Limit = 0;
const float drum_diameter = 8;		// [mm]
const float Torque_Constant = 23.724;	// [Nmm/A]
enum ERROR_STATE {
...
void HAL_FDCAN_RxFifo0Callback(FDCAN_HandleTypeDef *hfdcan, uint32_t RxFifo0ITs) {
	...
	if (hfdcan->Instance == hfdcan1.Instance) {
		if ((RxFifo0ITs & FDCAN_IT_RX_FIFO0_NEW_MESSAGE) != RESET) {
			...
			case TENSION_SENS_ID:
				memcpy(&curTension, &RxData[0], 4);
				heartbeat = true;
				break;

			case REF_TENSION_ID:
				memcpy(&recieved_tension, &RxData[0], 4);
...
//				/* Test program */
//				/*************************/
//				refCurrent =  refTension * (drum_diameter/2) / gearRatio / Torque_Constant;
...
//		MV = error[1] * Tension_P_gain + int_error * Tension_I_gain + refTension;		// PI control
